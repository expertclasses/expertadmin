var firebaseConfig = {
  ...
  function displayData(snapshot) {
    const dataList = document.getElementById('facultylistData');
    dataList.innerHTML = '';
    let serialNumber = 1;
    ...
    if (snapshot.exists()) {
      ...
      Object.keys(data).forEach(key => {
        const { Name, Degree, Experince,Age,Number,Email} = data[key];
        const div = document.createElement('div');
        div.innerHTML = `<p>${serialNumber}</p>
        <p>${Name}</p>
        <p id="deg">${Degree}</p>
        <p id="kok">${Experince}</p>
        <p id="lol">${Age}</p>
        <p id="pop">${Number}</p>
        <p>${Email}</p>
        `;
...
function fetchData() {
    const dataRef = database.ref('facultyform');
